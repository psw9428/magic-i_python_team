...
def initialize(timestamp):
    # 마우스 포인터를 따라 다니는 작은 검은 네모를 '먼저' 만든 다음...
...
def update(timestamp):
    ...
    if w.mouse_buttons[1]:
        # 메시지 출력할 때 매 번 str(cha) 같은 수식을 적는 것이 힘들다면
        # 아래와 같이 적는 것이 가능해요. 대충 써 보면 느낌 올 듯
        w.setTitle(f'1번 버튼을 누르고 있습니다 | 좌표: ({w.mouse_position_x}, {w.mouse_position_y})')
    else:
