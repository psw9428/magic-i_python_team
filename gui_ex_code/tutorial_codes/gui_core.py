...
- 이 파일 내용은 안 봐도 무방해요.
  ☆ 마크가 달려 있는 부분은 지금 시점에 납득하기 어려울 수 있으니 더더욱 안 봐도 돼요.

- 복잡한 프로그램을 만들 때는 이거보다 더 정교하고 효율적인 모듈들을 사용하게 될 거예요.
  이 모듈은 여러분이 '간단한 동작을, 내 손으로 직접' 다루어 보는 것을 목표로 구성되어 있고,
  그렇다 보니 일반적인 라이브러리들에 비해 제공하는 서비스(함수들 등)가 별로 없어요.
  잘 안 된다 싶으면, 인터넷 검색보다는(해봤자 잘 안 나옴) 톡방에 물어보거나 혼자 조금 더 고민해 보는 것을 권장해요.
...
class Window:
    ...
    def __init__(self, title='개발중!', width=800, height=600, interval=1/60, printKeysyms=False, printMouseButtonIdxs=False, isDebugMode=False):
        class Internals:
            '''
            ☆
            ...
            '''
            def __init__(self_internals, width, height, printKeysyms, printMouseButtonIdxs, isDebugMode):
                # Window object <- Internals object 연결
                self_internals.w = self
                
                self_internals.printKeysyms = printKeysyms
